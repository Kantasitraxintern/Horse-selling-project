...
// เปิด/สร้างฐานข้อมูล (ไฟล์จะอยู่ข้างๆ root โปรเจกต์)
...
app.post('/characters', (req, res) => {
  ...
  db.run(
    'INSERT INTO characters (name, va, color, image) VALUES (?, ?, ?, ?)',
    [name, va, color, image],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
...
app.put('/characters/:id', (req, res) => {
  const { name, va, color, image } = req.body || {};
  db.run(
    'UPDATE characters SET name=?, va=?, color=?, image=? WHERE id=?',
    [name, va, color, image, req.params.id],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      res.json({ id: Number(req.params.id), name, va, color, image, updated: this.changes });
    }
  );
});
